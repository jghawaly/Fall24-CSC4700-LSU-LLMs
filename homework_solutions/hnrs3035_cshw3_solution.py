...
import time
import os
from parse_squad_data import get_squad_questions
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Run GPT-4o-mini and Llama 3.1 8B on Squad v2 dev set. Uses GPt-4o for grading")
    ...
    qa_system_prompt = "You are an AI who is tasked with concisely answering questions/trivia. Only answer the question, don't add additional bloat content."
    ...
    if args.run_openai:
        ...
        with open("../data/squadv2_dev_gpt-4o-mini_answers_input_batch.jsonl", 'w') as jfile:
            for task in tasks:
                jfile.write(json.dumps(task) + '\n')

        # upload our batch file to OpenAI
        batch_file = openai_client.files.create(
            file=open("../data/squadv2_dev_gpt-4o-mini_answers_input_batch.jsonl", 'rb'),
            purpose='batch'
        )
        ...
        output_file_name = "../data/squadv2_dev_gpt-4o-mini_answers_output_batch.jsonl"
    ...
    if args.run_llama:
        ...
        for q, _ in tqdm(qas_we_want):
            ...
            response_objects.append({"custom_id": custom_id, "response": json.dumps(response['choices'][0]['message']['content'])})
            i += 1

        with open("../data/squadv2_dev_llama3-1-8b_answers_output_batch.jsonl", 'w') as jfile:
    ...
    if args.run_grader:
            ...
            with open(f_path, 'r') as file:
                for line in file:
                    ...
                    if 'body' in json_object['response']:
                        all_model_responses[json_object['custom_id']] = json_object['response']['body']['choices'][0]['message']['content']
                    else:
        ...
        load_and_store("../data/squadv2_dev_gpt-4o-mini_answers_output_batch.jsonl")
        load_and_store("../data/squadv2_dev_llama3-1-8b_answers_output_batch.jsonl")
        ...
        # Here, we are writing a local file to store the tasks. This is a jsonl file, newline delimited)
        with open("../data/squadv2_dev_scoring_input_batch.jsonl", 'w') as jfile:
            for task in tasks:
                jfile.write(json.dumps(task) + '\n')

        # upload our batch file to OpenAI
        batch_file = openai_client.files.create(
            file=open("../data/squadv2_dev_scoring_input_batch.jsonl", 'rb'),
            purpose='batch'
        )
        ...
        # Write the results to a local file, again, jsonl format
        result = openai_client.files.content(check.output_file_id).content
        output_file_name = "../data/squadv2_dev_scoring_output_batch.jsonl"
        with open(output_file_name, 'wb') as file:
            file.write(result)
    ...
    if args.view_scores:
        ...
        with open('../data/squadv2_dev_scoring_output_batch.jsonl', 'rb') as jfile:
            for line in jfile:
                # this converts the string into a Json object
                json_object = json.loads(line.strip())
        ...
        # show the results
        print(f"Llama 3.1 8B: {llama_correct_count} out of {llama_total_count}, or {llama_correct_count / llama_total_count:.2f}, questions answered correctly")
        print(f"GPT-4o-mini:  {openai_correct_count} out of {openai_total_count}, or {openai_correct_count / openai_total_count:.2f}, questions answered correctly")
